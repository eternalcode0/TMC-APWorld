...
from .constants import EXTERNAL_ITEM_MAP, GAME, WIND_CRESTS, TMCEvent, TMCFlagGroup, TMCItem, TMCLocation
...
def write_tokens(world: "MinishCapWorld", patch: MinishCapProcedurePatch) -> None:
    ...
    if world.options.goal.value == Goal.option_vaati:
        ...
        func = [0x00, 0x22, 0x05, 0x48, 0x04, 0x23, 0x03, 0x70, 0x42, 0x70, 0x82, 0x70, 0x01, 0x23, 0x8B, 0x71, 0x00,
                0x24, 0x78, 0x20, 0x01, 0x4B, 0x00, 0x00, 0x02, 0x10, 0x00, 0x03, 0xFF, 0x32, 0x05, 0x08]  # fmt:off
    ...
    write_bits(
        patch,
        0xFE0000,
        [world.options.goal.value == Goal.option_vaati, world.options.dhc_access == DHCAccess.option_open],
    )
    ...
    if options.big_octo_manipulation.value:
        ...
        patch.write_token(
            APTokenTypes.WRITE,
            0x0CE850,
            bytes([0x4A, 0xE8, 0x0C, 0x08, 0x4A, 0xE8, 0x0C, 0x08, 0x4A, 0xE8, 0x0C, 0x08, 0x4A, 0xE8, 0x0C, 0x08]),
        )
        # Disable Charge
    ...
    if options.replica_tod_boss_door.value:
        patch.write_token(
            APTokenTypes.WRITE,
            0x0E3950,
            bytes([0x06, 0x0F, 0x39, 0, 0, 0, 0, 0, 0x08, 0x01, 0x28, 0x00, 0xFF, 0xFF, 0x2C, 0x00]),
        )
    ...
    if world.options.shuffle_elements.value == ShuffleElements.option_dungeon_prize:
        ...
        prize_locs = {
            TMCLocation.DEEPWOOD_PRIZE: [[0xB2, 0x7A], [0x0D6C, 0x0AC0]],
        ...
        element_address = {
            TMCItem.EARTH_ELEMENT: 0x128699,
    ...
    if world.options.dhc_access.value == DHCAccess.option_closed and world.options.goal.value == Goal.option_vaati:
        ...
        ped_to_altar = Transition(
            warp_type=1, start_x=0xE8, start_y=0x28, end_x=0x78, end_y=0x168, area_id=0x89, room_id=0
        )
    ...
    if options.ped_figurines.value > 0:
        patch.write_token(
            APTokenTypes.WRITE,
            flag_group_by_name[TMCFlagGroup.FIGURINE_COLLECTED_FLAGS],
    ...
    red_fusion_requests = [0x2061, 0x2077, 0x2085, 0x208C, 0x2093, 0x215A, 0x21B6, 0x21BD, 0x2208, 0x2238, 0x2240, 0x2241, 0x2248, 0x2249, 0x2250, 0x2251, 0x2270, 0x2296, 0x2297, 0x229E, 0x22C8, 0x22E6, 0x22ED, 0x2310, 0x238B]  # fmt: off
    ...
    if (
        options.blue_fusion_access.value == FusionAccess.option_closed
        or options.blue_fusion_access.value == FusionAccess.option_open
    ):
        no_fusions_requests.extend(blue_fusion_requests)

    green_fusion_requests = [0x2062, 0x20AC, 0x20DD, 0x212E, 0x212F, 0x2130, 0x21C4, 0x21CB, 0x21D2, 0x21D3, 0x21DA, 0x21DB, 0x2200, 0x2207, 0x220F, 0x2216, 0x221D, 0x221E, 0x2231, 0x2260, 0x2261, 0x2285, 0x2286, 0x2287, 0x22A5, 0x22AC, 0x22B3, 0x22BA, 0x22CF, 0x22DD, 0x22DE, 0x22DF, 0x22FB, 0x2302, 0x2309, 0x233A, 0x23A0, 0x23A1, 0x23A8]  # fmt:off
    if options.green_fusion_access.value == FusionAccess.option_open:
    ...
    if (
        options.green_fusion_access.value == FusionAccess.option_closed
        or options.green_fusion_access.value == FusionAccess.option_open
    ):
    ...
    if world.options.goron_jp_prices.value:
        patch.write_token(
            APTokenTypes.WRITE,
            0x1112F0,
            struct.pack("<HHHHHHHHHHHHHHH", *[x for _ in range(0, 5) for x in [300, 200, 50]]),
        )
    ...
    for location_name, loc in location_table_by_name.items():
        ...
        if location_name in world.disabled_locations and (
            loc.vanilla_item is None
            or loc.vanilla_item in item_table
            and item_table[loc.vanilla_item].classification != ItemClassification.filler
        ):
