...
def fill_dungeon(world: "MinishCapWorld", dungeon: str, stage_items: frozenset[str], base_state: CollectionState):
    ...
    fill_locations = [
        loc for loc in world_locations if loc.name in location_groups[dungeon] and loc.name not in BANNED_KEY_LOCATIONS
    ]

    if len(fill_locations) < len(fill_stage_items):
        raise FillError(
            f"Not enough locations to pre_fill for slot '{world.player_name}', did plando or other "
    ...
    fill_restrictive(
        ...
        lock=True,
        allow_excluded=True,
        name=f"TMC Dungeon Fill: {dungeon}",
    )
